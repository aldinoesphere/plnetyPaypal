...
class PayPalInstallmentFinancingCosts
{
    ...
    public function call(
        ...
        if ($order instanceof Order) {
            if ($order->methodOfPaymentId == $paymentHelper->getPayPalMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALINSTALLMENT)) {
                $payments = $paymentRepositoryContract->getPaymentsByOrderId($order->id);

                $payment = $payments[0];

                if ($payment instanceof Payment && $payment->mopId == $paymentHelper->getPayPalMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALINSTALLMENT)) {
                    $creditFinancing = json_decode($paymentHelper->getPaymentPropertyValue($payment,
                        PaymentProperty::TYPE_PAYMENT_TEXT), true);
