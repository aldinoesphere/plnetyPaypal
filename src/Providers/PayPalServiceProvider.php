...
class PayPalServiceProvider extends ServiceProvider
{
                            ...
                            EventProceduresService $eventProceduresService)
    {
        ...
        $eventDispatcher->listen(ExecutePayment::class,
            ...
            {
                if( $paymentHelper->isPaymentOfTypePP($event->getMop()) )
                {
                    $paymentData = array();
                    $ppiMopId   = $paymentHelper->getPayPalMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALINSTALLMENT);
                    $pppMopId   = $paymentHelper->getPayPalMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALPLUS);
                    $ppeMopId   = $paymentHelper->getPayPalMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPALEXPRESS);
                    $ppMopId    = $paymentHelper->getPayPalMopIdByPaymentKey(PaymentHelper::PAYMENTKEY_PAYPAL);
                    ...
                    switch ($event->getMop())
                    {
                        case $ppiMopId:
                            ...
                            $paymentData['mopId'] = $ppiMopId;
                            break;
                        case $pppMopId:
                            $mode = PaymentHelper::MODE_PAYPAL_PLUS;
                            $paymentData['mopId'] = $pppMopId;
                            break;
                        case $ppeMopId:
                            $mode = PaymentHelper::MODE_PAYPALEXPRESS;
                            $paymentData['mopId'] = $ppeMopId;
                            break;
                        default:
                            $mode = PaymentHelper::MODE_PAYPAL;
                            $paymentData['mopId'] = $ppMopId;
                            break;
                    }
                    ...
                    if($paymentService->getReturnType() != 'errorCode')
                    {
                        ...
                        $plentyPayment = $paymentHelper->createPlentyPayment((array)$payPalPaymentData, $paymentData);
