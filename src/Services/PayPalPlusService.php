...
class PayPalPlusService extends PaymentService
{
     ...
     * @return string
     */
    public function getPaymentWallContent(Basket $basket, Checkout $checkout, CountryRepositoryContract $countryRepositoryContract, $withPppScript = true)
    {
        $country = 'DE';
        ...
        if($this->getReturnType() == 'redirectUrl')
        {
            ...
            $content .= '               "enableContinue": function(){
     ...
     * @return string
     */
    public function getPaymentWallContentByOrder(Order $order)
    {
        /** @var \Plenty\Modules\Order\Shipping\Countries\Contracts\CountryRepositoryContract $countryRepositoryContract */
        $countryRepositoryContract = pluginApp(\Plenty\Modules\Order\Shipping\Countries\Contracts\CountryRepositoryContract::class);
        ...
        $content = '';
        $approvalUrl = $this->getPaymentContentByOrder($order, PaymentHelper::MODE_PAYPAL_PLUS);
        if($this->getReturnType() == 'redirectUrl')
        {
            $content = '        <div id="ppplus"> </div>
                                <script type="application/javascript"> var ppp = PAYPAL.apps.PPP({
                                        "approvalUrl": "'.$approvalUrl.'", 
                                        "placeholder": "ppplus",
                                        "mode": "'.$mode.'",
