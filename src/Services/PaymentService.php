...
class PaymentService
{
    ...
    public function getPaymentContentByOrder(Order $order, $mode = PaymentHelper::MODE_PAYPAL, $additionalRequestParams=[]):string
    {
        ...
        $payPalRequestParams = $this->getPaypalParamsByOrder($order, $mode);
     ...
     * @return string
     */
    public function preparePayPalPaymentByOrder(Order $order)
    {
...
//        $settingsService->setSettingsValue(SettingsService::WEB_PROFILE, $webProfileResult);
     ...
     * @return array
     */
    public function getPaypalParamsByOrder(Order $order = null, $mode=PaymentHelper::MODE_PAYPAL)
    {
        $payPalRequestParams = $this->getApiContextParams($mode);
        ...
        foreach($order->orderItems as $orderItem)
        {
            ...
            $payPalBasketItem['itemId'] = $orderItem->variation->itemId;
            $payPalBasketItem['quantity'] = $orderItem->quantity;
            $payPalBasketItem['price'] = $orderItem->amounts[0]->priceGross;
        ...
        if($mode != PaymentHelper::MODE_PAYPAL_PLUS && $mode != PaymentHelper::MODE_PAYPALEXPRESS)
        {
            ...
            if(!is_null($shippingAddress))
            {
                ...
                // Get the URLs for PayPal parameters
                $payPalRequestParams['urls'] = $this->paymentHelper->getRestOrderReturnUrls($order->id);
